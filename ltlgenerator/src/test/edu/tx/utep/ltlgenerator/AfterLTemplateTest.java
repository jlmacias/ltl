...
public class AfterLTemplateTest {
	...
	public void testGlobalQPrecedesPcAfterLStar() throws Exception {
		String output = ltlGenerator.generateLTLFormula("QPrecedesPcAfterL", "P_AtLeastOneE_3", "Q_AtLeastOneH_3", "", "L_ConsecutiveC_3");
		assertEquals("!((!(l1 ^ X(l2 ^ X(l3)))) U ((l1 ^ X(l2 ^ X((l3 ^ (!(!((!(((q1 v q2 v q3) ^ (!(!p1 ^ !p2 ^ !p3) ^ ((!p1 ^ !p2 ^ !p3) U (p1 v p2 v p3)))))) U (!p1 ^ !p2 ^ !p3) ^ ((!p1 ^ !p2 ^ !p3) U (p1 v p2 v p3)))))))))))", output);
	}
	
	@Test
	public void testGlobalQPrecedesPcAfterLPlus() throws Exception {
		String output = ltlGenerator.generateLTLFormula("QPrecedesPcAfterL", "P_AtLeastOneE_3", "Q_ConsecutiveE_3", "", "L_ParallelC_3");
	...
	public void testGlobalQPrecedesPeAfterLStar() throws Exception {
		String output = ltlGenerator.generateLTLFormula("QPrecedesPeAfterL", "P_AtLeastOneE_3", "Q_AtLeastOneH_3", "", "L_ConsecutiveC_3");
		assertEquals("!((!(l1 ^ X(l2 ^ X(l3)))) U ((l1 ^ X(l2 ^ X((l3 ^ (!(!((!(((q1 v q2 v q3) ^ (!((!p1 ^ !p2 ^ !p3) v X(p1 v p2 v p3)))))) U ((!p1 ^ !p2 ^ !p3) v X(p1 v p2 v p3)))))))))))", output);
	}
	
	@Test
	public void testGlobalQPrecedesPeAfterLPlus() throws Exception {
		String output = ltlGenerator.generateLTLFormula("QPrecedesPeAfterL", "P_AtLeastOneE_3", "Q_ConsecutiveE_3", "", "L_ParallelC_3");
		assertEquals("!((!(l1 ^ l2 ^ l3)) U (((l1 ^ l2 ^ l3) ^ (!(!((!(((!q1 ^ !q2 ^ !q3) ^ (!((!p1 ^ !p2 ^ !p3) v X(p1 v p2 v p3)))) ^ (((!q1 ^ !q2 ^ !q3) ^ (!((!p1 ^ !p2 ^ !p3) v X(p1 v p2 v p3)))) U ((q1 ^ !q2 ^ !q3 ^ (!((!p1 ^ !p2 ^ !p3) v X(p1 v p2 v p3)))) ^ X((q2 ^ !q3 ^ (!((!p1 ^ !p2 ^ !p3) v X(p1 v p2 v p3)))) ^ X(q3)))))) U ((!p1 ^ !p2 ^ !p3) v X(p1 v p2 v p3))))))))", output);
	}
